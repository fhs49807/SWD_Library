	...
	public String loanReserveMedia(@RequestParam(required = false) Long mediaId,
		...
		if (!(authentication instanceof AnonymousAuthenticationToken)) {
			...
			if (mediaId == null || start_date == null || end_date == null) {
				return handleError("Please select a media item and specify the reserve date period.", start_date,
						end_date, selectedGenre, selectedType, username, model);
			}

			// Ensure start_date is before or equal to end_date
			if (start_date.after(end_date)) {
				return handleError("Start date must be on or before the end date.", start_date, end_date, selectedGenre,
			...
			try {
				...
				return handleError("Error processing loan/reservation: " + e.getMessage(), start_date, end_date,
